...
const isValidJWTFormat = (token) => {
  ...
  if (token === 'undefined') return { valid: false, reason: 'Token is literal string "undefined"' };
  if (token === 'null') return { valid: false, reason: 'Token is literal string "null"' };
  
  const parts = token.split('.');
  if (parts.length !== 3) {
    return { valid: false, reason: `JWT should have 3 parts (header.payload.signature), found ${parts.length}` };
  }
